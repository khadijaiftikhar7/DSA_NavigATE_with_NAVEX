...
void Graph::addNode(const string& name, double x, double y) {
...
string Graph::getLocation(int index) const {  
    if (index < 0 || index >= (int)Nodes.size()) return "Invalid";
    return Nodes[index]->getLocationName();
}

void Graph::addEdge(const string& sourceNode, const string& destinationNode, double weight) {
...
void Graph::removeEdge(const string& sourceNode, const string& destinationNode) {
    int u = getIndex(sourceNode);
    int v = getIndex(destinationNode);
...
void Graph::removeEdgesOfNode(const string& locationName) {
